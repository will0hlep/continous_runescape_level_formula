...
def levels_retrieve(username: str) -> list[list[int]]:
    """Retrieves a players levels and experiance values from Runescape's API"""
    data = []
    raw_data = requests.get(
        f'https://secure.runescape.com/m=hiscore/index_lite.ws?player={username}',
        timeout=2
        ).content
    for i, raw_skill in enumerate(raw_data.split()[1:len(SKILLS)+1]):
        skill = []
        for i in raw_skill.decode("utf-8").split(",")[1:]:
            skill.append(i)
        data.append(skill)
    return data
