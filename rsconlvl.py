...
parser = argparse.ArgumentParser(description='Calculates a float runescape level value based on the xp value input.')
parser.add_argument('xp', help='the integar amount of xp to be converted to a level value', type=float)
parser.add_argument("-e", "--elite", help="if present the formula for elite skills will be used", action="store_true")
args = parser.parse_args()

if args.elite:

    def elite_xp_calc(elite_level):
        elite = [0, 830, 1861, 2902, 3980, 5126, 6380, 7787, 9400, 11275,
                 13605, 16372, 19656, 23546, 28134, 33520, 39809, 47109, 55535,
                 65209, 77190, 90811, 106221, 123573, 143025, 164742, 188893,
                 215651, 245196, 277713, 316311, 358547, 404634, 454796,
                 509259, 568254, 632019, 700797, 774834, 854383, 946227,
                 1044569, 1149696, 1261903, 1381488, 1508756, 1644015, 1787581,
                 1939773, 2100917, 2283490, 2476369, 2679917, 2894505, 3120508,
                 3358307, 3608290, 3870846, 4146374, 4435275, 4758122, 5096111,
                 5449685, 5819299, 6205407, 6608473, 7028964, 7467354, 7924122,
                 ...
                 75303019, 77929820, 80618654, 83370445, 86186124, 89066630,
                 92012904, 95025896, 98106559, 101255855, 104474750, 107764216,
                 111125230, 114558777, 118065845, 121647430, 125304532,
                 129038159, 132849323, 136739041, 140708338, 144758242,
                 148889790, 153104021, 157401983, 161784728, 166253312,
                 170808801, 175452262, 180184770, 185007406, 189921255,
                 194927409]
        return elite[elite_level-1]


    def elite_power_fourier_series():
        ...
        for i in range(1,12):
            ...
            for j in range(series_size):
                ...
                elite_xp[j] = elite_xp_calc(polynomials[i][0] + j)
            ...
            for j in range(series_size):
                ...
                for k in range(1,int((series_size+1)/2)):
                    ...
                    A[j,2*k-1] = sin(2*pi*elite_level[j]*k/(series_size + (1 if i == 11 else 0)))
                b[j] = elite_xp[j]
    ...
    def elite_xp_continous(elite_level, elite_coeff = elite_power_fourier_series()):
        tier = 11
        ...
        for i in range(1,int((series_size+1)/2)):
            elite_xp += elite_coeff[2][tier][2*i]*cos(2*pi*elite_level*i/(series_size + (1 if tier == 11 else 0)))
            elite_xp += elite_coeff[2][tier][2*i-1]*sin(2*pi*elite_level*i/(series_size + (1 if tier == 11 else 0)))
        return elite_xp


    def elite_level_continous(xp):
        if xp < 0 or xp > 200000000:
            raise ValueError("xp must between 0 and 200m (inclusive)")
        level = 150
        xp_tmp = elite_xp_calc(level)
        while xp<xp_tmp:
            level -= 1
            xp_tmp = elite_xp_calc(level)
        if xp_tmp == xp:
            return level
        def func(level):
            return elite_xp_continous(level) - xp
        return min(max(level,fsolve(func,level)[0]),nextafter(level+1, float('-inf')))


    print(elite_level_continous(args.xp))
    ...
    def xp_continous(level, coeff = fourier_series()):
        xp = xp_approx(level)
        xp += coeff[0] * level
        for i in range(1,64):
